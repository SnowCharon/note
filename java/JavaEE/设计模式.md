...
![](../picture/customer_address.png)

在UML类图中单向关联用一个带箭头的实线表示。上图表示每个顾客都有一个地址，这通过让Customer类持有一个类型为Address的成员变量类实现。

**2，双向关联**

![](../picture/customer_product.png)
...
![image-20230319214204403](../picture/image-20230319214204403.png)
...
![](../picture/image-20191229173422328.png)

![image-20230319202159827](../picture/image-20230319202159827.png)
...
![](../picture/image-20191229173455149.png)

![image-20230319202210708](../picture/image-20230319202210708.png)
...
![](../picture/image-20191229173518926.png)

![image-20230319202354678](../picture/image-20230319202354678.png)
...
![](../picture/image-20191229173539838.png)

![image-20230319202932083](../picture/image-20230319202932083.png)
...
![](../picture/image-20191229173554296.png)

![image-20230319203450217](../picture/image-20230319203450217.png)

## 2.4 顺序图

### 2.4.1 顺序图定义与组成元素

顺序图（Sequence Diagram）是一种UML图，用于描述对象之间的动态交互。它展示了对象之间的消息传递顺序，并强调了时间的重要性
...
状态图（State diagram）是UML的一种图示方式，用于表示系统的状态和状态之间的转换。状态图可以描述系统的生命周期、对象的生命周期、对象的状态以及对象在状态之间的转换等。

状态图通常包括以下几个元素：

1. 状态（State）：系统或对象所处的状态。
2. 转移（Transition）：状态之间的转换。
3. 事件（Event）：导致状态转换的事件或条件。
4. 动作（Action）：在状态转换时执行的动作。

### 2.5.1 案例

![image-20230319213218862](../picture/image-20230319213218862.png)
...
单一职责原则(Single Responsibility Principle，SRP)是指**一个类只负责一项职责**，如果类承担的职责过多，就会导致可维护性变差，不易复用，容易变得庞大臃肿。因此应当遵循单一职责原则，将一个类的职责分解成多个类，每个类只负责一项职责，这样可以降低类的复杂度，提高代码的可读性和可维护性。
...
![image-20230319213754960](../../image-20230319213754960.png)
...
1. 避免了类爆炸式增长，继承会导致子类的爆炸式增长，而使用合成/聚合的方式可以有效的避免这种情况的发生。

2. 更加灵活，使用继承的方式会使对象之间的关系变的死板，而使用合成/聚合的方式可以使这种关系十分灵活。

3. 更容易扩展，使用合成/聚合的方式可以很容易的进行扩展，而使用继承的方式就不容易进行扩展了。

4. 减少耦合，通过使用合成/聚合的方式可以减少耦合，而使用继承的方式就会导致耦合性增加。

# 4，创建者模式

创建者模式（Builder Pattern）又叫生成器模式，是一种对象构建模式。它可以将复杂对象的建造
过程抽象出来，使这个抽象过程的不同实现方法可以构造出不同表现的对象。也就是说，它允许程序员将一个复杂的对象分解成多个简单的部件，然后再按某种算法组装成该复杂对象，就像组装汽车一样。

创建者模式主要包含四个角色：

1. Builder：抽象建造者，定义创建一个产品对象所需的各个部件的抽象接口。
2. ConcreteBuilder：具体建造者，实现Builder接口，构建和装配各个部件，定义并明确它所创建
    的表示，并提供一个检索产品的接口。
3. Director：导演者，负责安排复杂对象的建造次序，也就是组装模型，它不涉及具体的对象部件
    的建造。
4. Product：复杂对象，保存建造过程中创建的各个部件，它是组装模型的最终产品。
...
public class EagerSingleton {
     ...
     * @return 唯一的实例对象
     */
    public static synchronized EagerSingleton getEagerSingleton() {
...
public class EagerSingleton {
     ...
     */
    public static synchronized EagerSingleton getEagerSingleton() {
        //第一次判断，如果eagerSingleton不为null，不进入抢锁阶段，直接返回实例
        if(eagerSingleton == null) {
...
public class EagerSingleton {
     ...
     * 在成员位置创建该类的对象
     */
    private static volatile EagerSingleton eagerSingleton;
...
public class EagerSingleton {
    ...
    /**
     * 内部类
     * JVM在加载类的时候不会加载内部类，只有内部类的属性/方法被调用时才会被加载, 并初始化其静态属性
     */
     ...
     */
    public static synchronized EagerSingleton getEagerSingleton() {
        return EagerSingletonHolder.INSTANCE;
    }
}
```
    ...
    **Test类：**
    ...
    public class Test {
        public static void main(String[] args) throws Exception {
            ...
            //实例化对象
            EagerSingleton  eagerSingleton1 = (EagerSingleton)constructor.newInstance();
            EagerSingleton  eagerSingleton2 = (EagerSingleton)constructor.newInstance();
    ...
    public class EagerSingleton implements Serializable {
        ...
        private EagerSingleton() throws Exception {
            synchronized (EagerSingleton.class){
                //判断是否是第一次创建对象
                if (flag){
                    throw new Exception("不允许创建多个实例化对象");
                }
                //如果是第一次就放行，修改flag
                flag = true;
            }
...
<img src="../picture/工厂设计模式引入.png" style="zoom:80%;" />
...
<img src="../picture/简单工厂模式.png" style="zoom:70%;" />
...
<img src="../picture/工厂方法模式.png" style="zoom:70%;" />
...
<img src="../picture/image-20200401214509176.png" style="zoom:67%;" />

<img src="../picture/image-20200401222951963.png" style="zoom:67%;" />
...
<img src="../picture/抽象工厂模式.png" style="zoom:67%;" />
...
**简单工厂+配置文件解除耦合**——Spring的IOC很像貌似就是
...
<img src="../picture/JDK源码解析.png" style="zoom:75%;" />
...
用一个已经创建的实例作为原型，通过复制该原型对象来创建一个和原型对象相同的**新对象——无论是浅克隆还是深克隆都是新对象**
...
![原型模式的 UML 图](../picture/20201202-prototype-pattern.png)
...
<img src="../picture/原型模式1.png" style="zoom:80%;" />
...
<img src="../picture/原型模式2.png" style="zoom:80%;" />
...
<img src="../picture/原型模式3.png" style="zoom:80%;" />
...
<img src="../picture/image-20200413225341516.png" style="zoom:60%;" />
...
1. 首先，定义产品类，该类通常包含多个属性。
2. 接着，定义抽象建造者类，包含创建各个部件的抽象方法和返回最终产品的方法。
3. 然后，定义具体建造者类，实现抽象建造者类中定义的抽象方法，用于具体构造各个部件。
4. 接下来，定义指挥者类，负责控制建造过程，即按照一定的顺序调用建造者类的方法来构建复杂对象。
5. 最后，客户端调用指挥者类的方法来完成对象的创建。
...
<img src="../picture/建造者模式1.png" style="zoom:80%;" />
...
**工厂方法模式强调的是创建对象的过程的封装，而建造者模式强调的是创建复杂对象的过程的分离。在具体的应用场景中，需要根据实际情况来选择合适的设计模式。**
...
抽象工厂模式和建造者模式都是创建型设计模式，它们都可以用于对象的创建和构建，但是两者的侧重点不同。

抽象工厂模式的主要目的是提供一个创建一系列相关或相互依赖对象的接口，而不需要指定它们的具体类。这个接口定义了一些方法，用于创建一系列产品对象，而这些产品对象构成了一个产品族。在抽象工厂模式中，客户端不需要知道具体产品的类名，只需要知道具体工厂的类名即可。这使得系统更加灵活，因为可以在运行时动态地改变工厂来创建不同的产品族。因此，抽象工厂模式更加强调产品对象之间的关联性和一致性。

建造者模式的主要目的是将一个复杂对象的构建过程与其表示分离，使得同样的构建过程可以创建不同的表示。建造者模式通常适用于需要一步一步地创建复杂对象的场景，例如汽车的制造过程，对象的创建过程比较复杂，需要多个步骤，这些步骤可以有不同的顺序，并且每个步骤的实现细节也可能不同。在建造者模式中，我们将对象的构建过程分成多个步骤，由一个指挥者来协调这些步骤的顺序，并且使用不同的建造者来创建不同的表示。因此，建造者模式更加强调对象创建过程的细节和步骤，以及对象的创建过程和最终表示之间的分离。

因此，抽象工厂模式适用于需要一次性创建一整个产品族，而建造者模式适用于需要一步一步地构建一个复杂对象，并且允许在创建过程中改变对象的表示。

**抽象工厂模式适用于需要一次性创建一整个产品族，而建造者模式适用于需要一步一步地构建一个复杂对象，并且允许在创建过程中改变对象的表示。**
...
<img src="../picture/静态代理.png" style="zoom:80%;" />
...
<img src="../picture/适配器模式.png" style="zoom:80%;" />
...
<img src="../picture/对象适配器模式.png" style="zoom:80%;" />
...
<img src="../picture/装饰者模式.png" style="zoom:75%;" />
...
<img src="../picture/装饰者模式-jdk源码.png" style="zoom:80%;" />
...
![](../picture/image-20200207194617620.png)
...
<img src="../picture/桥接模式.png" style="zoom:80%;" />
...
<img src="../picture/外观模式.png" style="zoom:80%;" />
...
<img src="../picture/image-20200207234545691.png" style="zoom:60%;" />

RequestFacade类就使用了外观模式。先看结构图：

<img src="../picture/外观模式-jdk源码解析.png" style="zoom:70%;" />
...
<img src="../picture/image-20200208180417291.png" style="zoom:60%;" />
...
<img src="../picture/image-20200208182322313.png" style="zoom:80%;" />

要实现该案例，我们先画出类图：

<img src="../picture/组合模式.png" style="zoom:80%;" />
...
* 安全组合模式
    ...
    <img src="../picture/组合模式-安全性.png" style="zoom:80%;" />
...
<img src="../picture/俄罗斯方块.jpeg" style="zoom:60%;" />



**先来看类图：**

<img src="../picture/享元模式.png" style="zoom:80%;" />
...
<img src="../picture/image-20200208212930857.png" style="zoom:80%;" />
...
<img src="../picture/模板方法模式.png" style="zoom:80%;" />
...
<img src="../picture/image-20200210143039168.png" style="zoom:80%;" />

作为一个程序猿，开发需要选择一款开发工具，当然可以进行代码开发的工具有很多，可以选择Idea进行开发，也可以使用eclipse进行开发，也可以使用其他的一些开发工具。

<img src="../picture/image-20200210144457478.png" style="zoom:70%;" />
...
<img src="../picture/策略模式.png" style="zoom:80%;" />
...
<img src="../picture/image-20200211130313251.png" style="zoom:60%;" />
...
<img src="../picture/命令模式.png" style="zoom:75%;" />
...
<img src="../picture/责任链模式.png" style="zoom:80%;" />
...
<img src="../picture/状态模式前.png" style="zoom:80%;" />
...
<img src="../picture/状态模式.png" style="zoom:70%;" />
...
<img src="../picture/观察者模式.png" style="zoom:80%;" />
...
<img src="../picture/image-20200214110924010.png" style="zoom:60%;" />
...
<img src="../picture/中介者模式.png" style="zoom:70%;" />
...
<img src="../picture/迭代器模式.png" style="zoom:90%;" />
...
<img src="../picture/image-20200214215446638.png" style="zoom:70%;" />
...
<img src="../picture/image-20200426233931693.png" style="zoom:67%;" />
...
<img src="../picture/白箱备忘录模式.png" style="zoom:80%;" />
...
<img src="../picture/黑箱备忘录模式.png" style="zoom:70%;" />
...
<img src="../picture/image-20200215220322641.png" style="zoom:60%;" />
...
<img src="../picture/image-20200215225227616.png" style="zoom:50%;" />
...
<img src="../picture/解释器模式.png" style="zoom:80%;" />
...
* 编写配置文件。在类路径下编写一个名为ApplicationContext.xml的配置文件
    ...
    <img src="../picture/image-20200429165544151.png" style="zoom:60%;" />
...
<img src="../picture/image-20200429111324770.png" style="zoom:40%;" />
...
<img src="../picture/image-20200429185050396.png" style="zoom:60%;" />
...
<img src="../picture/image-20200430220155371.png" style="zoom:60%;" />
...
<img src="../picture/image-20200429204239868.png" style="zoom:60%;" />
...
<img src="../picture/image-20200429204700956.png" style="zoom:60%;" />
...
<img src="../picture/image-20200429211132185.png" style="zoom:60%;" />
